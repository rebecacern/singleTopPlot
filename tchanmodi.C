...
void tchanmodi(){
  ...
  labelsingle->AddText("Single top, t-channel production");
  ...
  double tch_central[6], tw_central[6], tch_lower[6], tch_upper[6], tw_lower[6], tw_upper[6], x[6];
  int j = 0;
  for (int i = 0; i< 6; i++){
    j = i + 8;
    if (i > 2) j = i +9;
  ...
  double etch_lower[6], etch_upper[6], etw_lower[6], etw_upper[6];
  for (int i=0; i< 6; i++){
  ...
  gr = new TGraphAsymmErrors(6,x,tch_central,0,0, etch_lower, etch_upper);
  ...
  grl = new TGraph(6,x,tch_central);
  ...
  grtw = new TGraphAsymmErrors(6,x,tw_central,0,0, etw_lower, etw_upper);
  ...
  grtwl = new TGraph(6,x,tw_central);
  ...
  gr_tch_8TeV_comb = new TGraphAsymmErrors(1,tch_8TeVx_comb,tch_8TeV_comb,0,0, tch_8TeVerror_comb, tch_8TeVerror_comb);
  gr_tch_8TeV_comb->SetMarkerColor(kMagenta);
  gr_tch_8TeV_comb->SetMarkerStyle(23);
  gr_tch_8TeV_comb->SetMarkerSize(1.5);
  ...
  mgd->Add(gr_tch_7TeV);
  mgd->Add(gr_tch_8TeV);
  // mgd->Add(gr_tch_8TeV_comb);
  // mgd->Add(gr_tw_7TeV);
  // mgd->Add(gr_tw_8TeV);
  mgd->Draw("P");
  
  mg->GetYaxis()->SetRangeUser(10,130); 
  ...
  leg = new TLegend(0.18,0.15,0.8,0.4);
  leg ->SetFillStyle(1001);
  leg ->SetFillColor(kWhite);
  leg ->SetTextSize(0.035);
  leg ->SetTextFont(42);
  leg ->SetBorderSize(0);
  leg ->SetHeader("t-channel production");
  ...
  // leg->AddEntry(gr_tch_8TeV_comb, "LHC combination, PAS-TOP-12-002, ATLAS-CONF-2013-098", "p");
  ...
  mg->GetYaxis()->SetRangeUser(20,160); 
  mg->GetXaxis()->SetNdivisions(4,kTRUE); 
  c1->Update();
  c1->SaveAs("plots/tchan_modi.png");
